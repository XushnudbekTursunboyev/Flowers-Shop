...
import uz.xushnudbek.flowersshop.databinding.FragmentRegisterBinding
        ...
        viewModel.register.observe(viewLifecycleOwner, Observer { response ->
            when (response) {
                ...
                is Resource.Success -> {
                    ...
                    Toast.makeText(activity, resources.getText(R.string.signed_up_successfully), Toast.LENGTH_SHORT).show()
                    viewModel.logOut()
                    findNavController().navigate(R.id.action_registerFragment_to_loginFragment)
